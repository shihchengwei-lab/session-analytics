...
一個 [Agent Skill](https://agentskills.io)，讓 AI coding agent 持續改進你們的協作方式——定位是**自我優化的 harness 技能**：每個 agent 分析**自己這套工具**最近 7 天的本地 session 紀錄，把你真正在用的路徑加粗、沒在用的剪枝，並在模型更換後重新驗證自己過去給的建議——讓整套設定持續進步，而不是越堆越多。所有資料都留在你的機器上。
