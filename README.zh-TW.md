...
- **週回顧**：總覽之外，在三條戰線上提出證據綁定的改動：
  ...
  - 每個提案先過機械驗證器（標記邊界位元組級比對、規則數上限）才給你看完整 diff——**你核准之後才寫入**；標記範圍外的內容永遠不碰
...
抽取器與驗證器的測試跑在合成 log 與設定檔上、只用標準函式庫：`python -m pytest tests/`（或 `python -m unittest discover tests`）。
...
- 資料集檔案含有你的 prompt 與專案路徑——輸出請視為私人資料。
