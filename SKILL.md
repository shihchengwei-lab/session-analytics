...
2. Re-derive the **entire block** against this week's evidence — every existing rule gets a verdict:
   ...
   - **retire** — target friction absent for 2 consecutive reviews → graduated. If retiring regresses, the next review brings a rewritten rule back — the loop self-corrects, so retire without fear. Consecutive means on-cadence: after a multi-week gap between reviews, absence is missing data, not absent friction — re-validate instead of retiring.
...
5. **Skill & config hygiene — bolden the used, prune the unused**: cross the installed-skill inventory (the agent sees its own available-skills list; disk locations per your reference) against invocation counts, both paths summed (`skill_counts` + `/name` in `command_counts` — a skill only ever typed by hand looks dead on tool calls alone; some are *configured* manual-only, e.g. Claude Code's `disable-model-invocation: true`, making their `skill_counts` structurally zero — check the skill's frontmatter before reading zero as disuse). Where the tool ships its own setup audit (Claude Code: `/checkup`, alias of `/doctor`, flags unused skills/MCP servers/plugins against their context cost), run it as a cross-check — its "unused" detection basis is undocumented (as of 2026-07), so it complements this invocation evidence rather than replacing it. Then, user decides:
   ...
   - **never invoked in the wide window** → candidate to disable/remove; name the count and window. Guards: a skill younger than the window has no evidence either way — skip it; ambient plugins (statuslines, hooks, MCP servers) work without being "called" — never judge them by invocation counts; a situational skill may earn its keep rarely — state what it's for, let the user judge; a cadence skill — this one included — runs on a schedule, not on demand: zero invocations means the loop stalled, so propose restoring the trigger (re-arm the schedule, pair it with an independent reminder), never retirement for idleness.
