...
- **Weekly review**: the overview, plus maintenance of a managed rules block in the tool's config (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`):
  - hard cap 10 one-line rules, each carrying its evidence and dates
  - every week the whole block is re-derived: keep / rewrite (friction recurred despite the rule) / merge (overlap) / retire (2 clean weeks = graduated) — it refactors instead of accumulating
...
| Tool | Source | Cost |
|---|---|---|
| Claude Code | `~/.claude/usage-data` (run `/insights` first) | precomputed — free to read |
...
- Claude Code path: `/insights` run at least once; Codex path: existing session logs

## Status

Early release. The analysis paths are tested against real data (Claude Code artifacts; Codex logs at the data layer). The weekly loop's multi-week behavior — rules being rewritten, merged, and retired across consecutive reviews — is specified but has not yet been exercised in the field, and the managed-block write path always requires your approval. Treat it as a v0 you watch, not an autopilot.
